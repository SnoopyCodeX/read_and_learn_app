...
class ClassroomStoryListPanel extends StatefulWidget {
  ...
  @override
  _ClassroomStoryListPanelState createState() =>
      _ClassroomStoryListPanelState();
}
...
class _ClassroomStoryListPanelState extends State<ClassroomStoryListPanel> {
                    ...
                    Flexible(
                      ...
                      child: StaggeredGridView.countBuilder(
                        ...
                        itemBuilder: (context, index) => _buildStoryCard(
                            stories[index], unlockedStories[index]),
                      ),
                    ),
                  ],
                ),
  ...
  Future<Result<List<Story>>> _getStories() async {
    ...
    Result<List<Story>> stories =
        await StoryService.instance.getStory('classroom', widget.classId);
    Result<List<UserProgress>?> progresses = await UserProgressService.instance
        .getAllFinishedProgress(widget.classId, userData['id']);
    unlockedStories = [];

    if (!stories.hasError) {
      if (!progresses.hasError) {
        print(
            'Progress Count: ${(progresses.data as List<UserProgress>).length}');

        // Lock all stories by default
        // ignore: unused_local_variable
        for (Story story in stories.data!) unlockedStories.add(false);
        ...
        if (unlockedStories.isNotEmpty) unlockedStories[0] = true;
      }
    } else {
      // ignore: unused_local_variable
      for (Story story in stories.data!) unlockedStories.add(false);
