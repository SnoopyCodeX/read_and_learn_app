...
class ParentListView extends StatefulWidget {
  ...
  @override
  _ParentListViewState createState() => _ParentListViewState();
}
...
class _ParentListViewState extends State<ParentListView> {
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        if(snapshot.connectionState == ConnectionState.done && snapshot.hasData)
        {
          ...
          if(!data.hasError)
          {
            List<User> parents = data.data as List<User>;
            List<User> temp = [...parents];

            for(User user in temp)
              if(user.id == widget.parentId) {
                parents.remove(user);
                break;
              }

            List<User> _searchList = [];
            for(User user in parents)
  ...
  Widget _buildParentCard(User user, int count) {
    return Container(
  ...
  void _showDeleteDialog(User user) {
    Utils.showAlertDialog(
      ...
      message: 'Do you really want to delete this parent?', 
      actions: [
        TextButton(
          onPressed: () => _deleteParent(user), 
          child: Text(
            'Yes',
            style: GoogleFonts.poppins(
  ...
  void _showEditDialog(User user) {
    Utils.showAlertDialog(
      context: context, 
      title: 'Confirm Action', 
      message: 'Do you really want to edit this parent?', 
      actions: [
        TextButton(
          onPressed: () {
  ...
  Future<void> _deleteParent(User user) async {
    Navigator.of(context, rootNavigator: true).pop();

    Utils.showProgressDialog(
      context: context, 
      message: 'Deleting parent...',
    );
