...
class UserProgressService {
  ...
  static UserProgressService get instance => UserProgressService();
  ...
  Future<Result> resetUserProgress(
      String userId, String classId, String storyId) async {
    List<Map<String, dynamic>> data = await _firestoreService.findData(
      USER_PROGRESS_TABLE,
      key: 'class_id',
      isEqualTo: classId,
  ...
  Future<Result<List<UserProgress>?>> getUserProgress(
  ...
  Future<Result<List<UserProgress>?>> getAllFinishedProgress(
    ...
    if (data.isNotEmpty)
      for (Map<String, dynamic> json in data) {
        ...
        if (progress.userId == userId &&
            progress.status == STATUS_FINISHED_READING)
  ...
  Future<Result<List<UserProgress>?>> getAllNotFinishedProgress(
      String classId, String userId) async {
    List<Map<String, dynamic>> data = await _firestoreService.findData(
      USER_PROGRESS_TABLE,
      key: 'class_id',
    ...
    if (data.isNotEmpty)
      for (Map<String, dynamic> json in data) {
        ...
        if (progress.userId == userId &&
            progress.status == STATUS_STILL_READING) userProgress.add(progress);
      }
  ...
  Future<Result<List<UserProgress>?>> getAllFinished(
      String classId, String storyId) async {
    List<Map<String, dynamic>> data = await _firestoreService.findData(
      USER_PROGRESS_TABLE,
      key: 'class_id',
      isEqualTo: classId,
  ...
  Future<Result<List<UserProgress>?>> getAllNotFinished(
      String classId, String storyId) async {
    List<Map<String, dynamic>> data = await _firestoreService.findData(
      USER_PROGRESS_TABLE,
      key: 'class_id',
  ...
  Future<Result<List<UserProgress>?>> getProgress(
    ...
    return Result<List<UserProgress>?>(
        data: data.isNotEmpty
            ? data.map((json) => UserProgress.fromJson(json)).toList()
            : null,
        hasError: data.isEmpty);
  }
}
